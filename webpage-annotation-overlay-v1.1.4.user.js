  ...
  function openCollectionOverview() {
    ...
    const pageRows = pages.length ? pages.map(page => `
      <article class="fi-collection-page">
        ...
        <div class="fi-collection-page-actions">
          <button type="button" data-open-url="${escapeAttr(page.page?.url || page.pageKey)}">Open</button>
          <button type="button" class="fi-delete-page" data-delete-page="${escapeAttr(page.pageKey)}">Delete</button>
        </div>
    ...
    modal.querySelectorAll('[data-delete-page]').forEach(button => button.addEventListener('click', () => {
      deleteCollectionPage(button.dataset.deletePage);
    }));
  }

  function deleteCollectionPage(pageKey) {
    const entry = state.pageIndex.find(item => item.pageKey === pageKey);
    if (!entry) {
      toast('That page record is no longer available');
      openCollectionOverview();
      return;
    }

    const record = readStoredPageRecord(pageKey);
    const collection = state.collections.find(item => item.id === entry.collectionId) || activeCollection();
    const title = record?.page?.title || entry.page?.title || pageKey;
    const annotationCount = Array.isArray(record?.annotations)
      ? record.annotations.length
      : Number(entry.annotationCount || 0);
    const hasSnapshot = Boolean(record?.snapshot || entry.hasSnapshot);
    const annotationLabel = `${annotationCount} annotation${annotationCount === 1 ? '' : 's'}`;
    const snapshotLabel = hasSnapshot ? ' and its saved snapshot' : '';

    if (!confirm(
      `Delete “${title}” from “${collection?.name || 'this collection'}”?

` +
      `This permanently deletes the saved page record, ${annotationLabel}${snapshotLabel}. ` +
    ...
    GM_deleteValue(storageKey(pageKey));
    state.pageIndex = state.pageIndex.filter(item => item.pageKey !== pageKey);
    saveWorkspace();
    renderAll();
    toast(`Page deleted · ${annotationLabel} removed`);
    openCollectionOverview();
  }
